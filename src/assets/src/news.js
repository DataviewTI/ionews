  ...
  function(self){
    ...
    $('#keywords').pillbox({
      ...
      onAdd:function(data,callback){
        var els=[];
        $('#keywords').pillbox('items').forEach(function(a){
          els.push(a.value+'');
        });
        if(!els.includes(data.value))
          $('#keywords').pillbox('addColorItem',{text:data.text,value:data.value},'random',true);
          //callback(data);
        else
          toastr['error']('não informe palavras chave duplicadas!');
      },
      onRemove:function(data,callback){
        if(data.el.attr('tip')=='true')
        {
          $(document.createElement('span')).text(data.value).on('click',function(e){
    ...
    self.dt = $('#default-table').DataTable({
      ...
      [
        ...
        {targets:'__dt_c', searchable:false,width:"2%",orderable:false,className:"text-center",render:function(data,type,row){
        ...
        {targets:'__dt_d',width: "2%",orderable:true,className:"text-center",render:function(data,type,row){
            if(data)
              return self.dt.addDTIcon({ico:'ico-image',title:'notícia com imagens',value:1,pos:'left',_class:'text-success'});
            else
        ...
        {targets:'__dt_f',width: "2%",orderable:true,className:"text-center",render:function(data,type,row){
            if(data)
              return self.dt.addDTIcon({ico:'ico-star',value:1,title:'notícia destaque',pos:'left',_class:'text-info'});
            else
              return self.dt.addDTIcon({value:0,_class:'invisible'});
          }
        },
    ...
    self.wizardActions(function(){
        var _kw=[];
        $('#keywords').pillbox('items').forEach(function(a){_kw.push(a.value);});
...
function view(self){
  return{
      onSuccess:function(data){
          ...
          $("[name='date']").pickadate('picker').set('select',new Date(data.date));
          ...
          if(data.keywords!=null){
            ...
            $('#keywords').pillbox('addColorItem',{text:kw,value:kw},'random',false);
              $(this).remove();
            });
          }
